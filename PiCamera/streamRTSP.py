    ...
    def beginStream():
        if foundWifi:   # Only begin the stream if wifi is detected
            ...
            with open(curDir + 'streamLogs.txt', 'w+') as logs:    # Errors (although everything from streaming is interpreted as an error)
                firstCommand = subprocess.Popen(["""cvlc -vvv v4l2c:///dev/video0:width=640:height=480:chroma=H264 --sout '#rtp{sdp=rtsp://:8554/}' --demux h264"""],
                ...
                try:
                    ...
                    firstCommand.kill()
                secondCommand = subprocess.Popen(["""dd if=/dev/video0 bs=1M | cvlc -vvv stream:///dev/stdin --sout '#rtp{sdp=rtsp://:8554/}' --demux=h264"""],
                                                 stderr=logs, shell=True)
